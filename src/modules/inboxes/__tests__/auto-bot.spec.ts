 ...
 *   select(...).from(...).where(...).limit(1)            — inbox, playbook, bot lookups
 ...
 *
 * `selectLimits` feeds the three lookups in order: [inbox], [playbook], [bot].
...
function playbookRow() {
  return { inboxId: INBOX_ID, content: 'a'.repeat(40), etag: 'etag1', version: 1 };
}
...
describe('applyAutoBotForInbox', () => {
  it('creates a builtin bot when playbook + key + provider are all present', async () => {
    const { db, spies } = makeDb({
      selectLimits: [[inboxRow()], [playbookRow()], []],
    ...
    expect(botValues.config.playbookSource).toBe('local');
    expect(botValues.config.systemPrompt).toBe('a'.repeat(40));
  ...
  it('disables the bot when the playbook is removed', async () => {
    const { db, spies } = makeDb({
    ...
    expect(setCalls.some((s) => s.defaultBotId === null)).toBe(true);
  ...
  it('disables the bot and clears columns when the key is removed (newApiKey=null)', async () => {
    const { db, spies } = makeDb({
      selectLimits: [[inboxRow({ botLlmApiKeyEnc: 'enc', botLlmProvider: 'anthropic', defaultBotId: BOT_ID })], [playbookRow()], [botRow()]],
    });
  ...
  it('rotates the secret when a new key is provided for an active bot', async () => {
    const { db, spies } = makeDb({
      selectLimits: [[inboxRow({ botLlmApiKeyEnc: 'old-enc', botLlmProvider: 'anthropic', defaultBotId: BOT_ID })], [playbookRow()], [botRow()]],
    });
  ...
  it('disables the bot when the inbox is soft-deleted', async () => {
    const { db, spies } = makeDb({
      selectLimits: [[inboxRow({ botLlmApiKeyEnc: 'enc', botLlmProvider: 'anthropic', defaultBotId: BOT_ID, deletedAt: new Date('2026-06-01T00:00:00Z') })], [playbookRow()], [botRow()]],
    });
