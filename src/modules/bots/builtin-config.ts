...
export const builtinBotConfigSchema = z.object({
  ...
  /** O prompt vem SEMPRE de cfg.systemPrompt. Campo mantido só pra retrocompat com bots antigos: qualquer valor legacy ('atlas', 'local') é normalizado pra 'inline' pelo .catch. */
  playbookSource: z.literal('inline').catch('inline').default('inline'),
